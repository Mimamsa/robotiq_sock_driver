...
class RobotiqGripper:
    ...
    def deactivate(self):
        """ """
        set_ok = self._set_var(self.ACT, 0)
        return set_ok
    ...
    def activate_emergency_release(self, open_gripper=True):
        """ """
        self._set_var(self.ATR, 0)        
        release_dir = 0 if open_gripper else 1  # release direction - 0: open, 1: close
        var_dict = OrderedDict([(self.ATR, 0), (self.ARD, release_dir)])
        set_ok = self._set_vars(var_dict)
        return set_ok
    
    def deactivate_emergency_release(self):
        """ """
        set_ok = self._set_var(self.ATR, 0)
        return set_ok
